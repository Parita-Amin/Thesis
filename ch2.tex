...
In 2006, 
Berners-Lee\footnote{\tt https://www.w3.org/DesignIssues/LinkedData}
...
linked data 
principles, and have been enhanced by Berners-Lee, Bizer, and Heath~\cite{bizer2009emerging, bizer2011linked, heath2011linked}.
...
characterized a \ac{uri} by discussing the component terms:
Numerous advantages come from {\bf uniform}ity. 
...
A {\bf resource} is anything that can be identified
...
An {\bf identifier} distinguishes an object from all other objects within its scope.
...
%the
true for some.
A system using \acp{uri} is not required to access the resource identified
...
(see Statements~\ref{generic-uri} and~\ref{authority-uri}). 
...
\end{figure}
\noindent
One of the \acp{uri} from the Survey ontology is presented in Statement~\ref{example-uri}.
Figure~\ref{components-of-uri-example} shows its components.
\begin{flalign}
...
A statement, 
in general terms, 
can be seen as a building block of a statement in English, 
...
The University of Manchester developed the 
Pizza ontology~\cite{horridge2011practical,debellis2021practical}, 
...
\begin{figure}[h]
	...
	\framebox{\vbox{\tt\scriptsize
        \begin{tabbing}
            ...
            \>"xmlns:pizza="http://www.semanticweb.org/pizzatutorial/ontologies/2020/PizzaTutorial\#"\\
      ...
      \caption{Statement~\ref{caloriccontent} expressed as \acs{rdf}/\acs{xml}. The first few lines 
      define prefixes as a shorthand for the \acp{uri}. }
      \label{rdf-xml-doc}
\end{figure}

\begin{flalign}  
    \text{\tt <pizza:Pizza>  <pizza:hasCaloricContent>  <xsd:integer>}&&
    \label{rdf-triple}
\end{flalign}
\noindent
Statement~\ref{rdf-triple} 
shows an example of an \ac{rdf} triple, 
where the subject (resource) is {\tt pizza:Pizza}; 
the predicate (property) is {\tt pizza:hasCaloricContent}; 
and the object (property value) is {\tt xsd:integer}. 

Figure~\ref{rdf-graph} 
shows Statement~\ref{caloriccontent} in graphical format. 
...
\begin{figure}[htp]
    ...
    \caption{Graphical representation of Statement~\ref{caloriccontent}.}
    \label{rdf-graph}
\end{figure}
    
The subject and object of an \ac{rdf} triple are nodes in a graph
and the predicate is an edge that connects the nodes. 
...
formats, 
the \ac{rdf} data model has a variety of serialization formats 
including
\ac{n3} and \ac{turtle}.
\ac{n3} is an extension of N-Triples (line based) 
and adds a graph label to each line 
that allows multiple graph encoding~\cite{cyganiak2014rdf}.

There can be more details added to Statement~\ref{caloriccontent}
(illustrated in Figures~\ref{rdf-xml-doc} and~\ref{rdf-graph}) and
Statement~\ref{rdf-triple}. 
That is, 
...
connects the resource (subject,\\{\tt pizza:Pizza}) 
with the value (object, {\tt pizza:PizzaBase}).
...
a description of the types of {\tt PizzaBase} can be included.
...
\begin{figure}[ht]
    ...
    \caption{Graphical representation of Statements~\ref{caloriccontent} and~\ref{pizzabase} combined,
    following the \ac{rdf} 1.1 Primer~\cite{schreiber2014rdfprimer}.}
    \label{graph-for-relative-statements}
\end{figure}
\noindent

\begin{flalign} 
    \text{\tt PizzaBase has types DeepPanBase and ThinAndCrispyBase} &&
    \label{kindsofbase}
\end{flalign}
...
\begin{figure}[ht]
    ...
    \caption{Graphical Representation of \ac{rdf} with types of {\tt PizzaBase}, 
    after the \ac{rdf} 1.1 Primer~\cite{schreiber2014rdfprimer}.}
    \label{graph-for-base-types}
\end{figure}

The {\tt pizza:Pizza} (subject) node
is connected to the {\tt pizza:PizzaBase} (object) node 
through the {\tt pizza:hasBase} (predicate). 
Further, 
...
The relationships of the blank node with the 
other nodes give sufficient details 
without hindering the flow of the data tree.
...
\begin{figure}[htp]
    ...
    without a \ac{uri} (blank node) for {\tt PizzaBase} Entity, after the \ac{rdf} 
    1.1 Primer~\cite{schreiber2014rdfprimer}.}
    \label{blank-node-for-rdf}
\end{figure}

According to the \ac{w3c} document
{\em RDF 1.1 Concepts and Abstract Syntax}\footnote{\tt https://www.w3.org/TR/rdf11-concepts/},  
the design should 
...
Some of the commonly used \ac{rdfs} 
classes and properties\footnote{\tt https://www.w3.org/TR/rdf-schema/} 
along with their description 
are listed in Table~\ref{rdfs-classes-and-properties}~\cite{mcbride2004resource}.
...
\begin{table}[h!]
    ...
    \begin{tabular}{| l | p{10cm} |} \hline
     ...
     rdfs:range & An instance of rdf:Property that is used to state that 
     the values of a property are instances of one or more classes \\ \hline
     rdfs:domain & An instance of rdf:Property that is used to state that 
     any resource that has a given property is an instance of one or more classes \\ \hline
     rdf:type & An instance of rdf:Property that is used to state that 
     a resource is an instance of a class  \\ \hline
     rdfs:subClassOf & An instance of rdf:Property that is used to state that 
     all the instances of one class are instances of another  \\ \hline
     rdfs:label & an instance of rdf:Property that may be used to provide 
     a human-readable version of a resource's name  \\ \hline
     rdfs:subPropertyOf \ \ & An instance of rdf:Property that is used to state that 
     all resources related by one property are also related by another  \\ \hline
    \end{tabular}
...
According to the Open Knowledge 
Foundation\footnote{\tt https://opendatahandbook.org/guide/en/what-is-open-data/}, 
...
Azogu and Longo~\cite{open2022} say that 
``data and information that is ... electronically available in a 
machine-readable format, is freely accessible by anyone and may be used, reused or redistributed without 
reasonable restriction''.
To make open data available to everyone on the web, 
the data do not need to be interlinked. 
...
To control the use of open data, 
Berners-Lee 
proposed a protocol called SOcial LInked Data (SOLID)~\cite{soliddemo2016,solidtutorial2016}
which focuses on web decentralization. 
The project identifies the need for control over the use of available data 
by web economy firms like social media sites.
He suggests holding the available linked data in cloud-based pods 
that can be controlled by data developers/owners who can grant access 
but would still have the control over their data. 
...
have been developed.
Wikidata is one of the major projects of linked open data that 
present structured data on the semantic web. 
...
into
the semantic web. 
Wikidata is a free and open-source tool 
that handles a large amount of 
structured data and maintains pages that store the data. 
Each entity on Wikidata represents 
the subject of a triple with a dedicated page. 
The properties of Wikidata are similar to that of \ac{rdf} 
where the individuals and classes are indicated using items. 
Data on a page of Wikidata is available 
for the users to access as well as edit.
...
Wikidata uses a method of automatically assigning 
unique identifiers to the entities, 
which are used as the labels of the item pages 
instead of the actual names of the entities. 
These identifiers are a string starting with a capital Q 
followed by a sequence of digits. 
The identifiers are assigned irrespective of the language 
as the Wikidata website supports multiple languages.
...
The properties, along with their property IDs, 
used for the University of Regina 
are listed in Table~\ref{uofr-statement-properties-and-id}.
The other section is Identifiers, which consists of 
various IDs associated with the subject.
The identifiers, along with their property IDs, 
used for the University of Regina 
are listed in Table~\ref{uofr-identifier-properties-and-id} 
whereas the \ac{rdf}/\ac{xml} 
description of the University of Regina 
on Wikidata is as shown in Figure~\ref{rdf-xml-uofr}. 
  
\begin{table}[h!]

		\centering
		
		\caption{Statement Properties and Property IDs for the University of Regina on Wikidata.\label{uofr-statement-properties-and-id}}
...
\begin{figure}[htp]
    ...
    \begin{tabbing}
        ...
        \>\>\><wdt:P18 rdf:resource="http://commons.wikimedia.org/wiki/Special:\\\>\>\>\>FilePath/University\%20of\%20Regina\%20Library\%20viewed\%20from\%20the\%20Oval.jpg"/>\\
        \>\></rdf:Description>\\
        \></rdf:Description>\\
        </rdf:RDF>
        \\
    ...
    \caption{Wikidata Entry (RDF/XML) for {\tt University of Regina}.}
    \label{rdf-xml-uofr}
\end{figure}
...
suitable concept for particular definitions~\cite{horridge2011practical}.
...
\begin{figure}[htp]
    ...
    \caption{Representation of properties and individuals in OWL.}
...
In \ac{owl}, 
the sets consisting of individuals are called classes. 
...
``Subclasses specialize (``are subsumed by'') their super-classes''~\cite{horridge2011practical}. 
For instance, 
...
\begin{itemize}
    ...
    \item All customers are persons
    
    \item All members of class {\tt Customer} are members of class {\tt Person}
    
    \item Being a {\tt Customer} implies that individual is a {\tt Person}
...
\begin{figure}[htp]
    ...
    \caption{Representation of classes, individuals, and properties in OWL.}
...
Ontology editors are used by data publishers 
to create new ontologies when existing vocabularies are not suitable for their purpose. 
Of the ontology editors listed on the \ac{w3c} 
wiki page\footnote{\tt https://www.w3.org/wiki/Ontology\_editors}
that appear to be active with a current cross-platform application available
(Protégé, Neologism, TopBraid Composer, Anzo, Semantic Turkey, and VocBench),
Protégé\footnote{\tt https://protege.stanford.edu/} 
is the most popular amongst researchers.
Protégé also has a well-established tutorial~\cite{horridge2011practical,debellis2021practical} explaining its use.

\section{Linked Data Life Cycle}
\label{sec:linkeddatalifecycle}

The Linked Data Life Cycle described by 
Ngomo et al.~\cite{ngomo2014introduction}
includes multiple stages, which are listed in
Table~\ref{ldlc-stages}.
It is not required to go through all the stages sequentially 
to publish \ac{lod} on the web. 
The first stage 
is extraction of non-\ac{rdf} data and conversion of that data 
into \ac{rdf}. 
Then,
...
\begin{table}[h!]
    ...
    \caption{Stages in the Linked Data Life Cycle described by Ngomo et al.~\cite{ngomo2014introduction}}
    \label{ldlc-stages}

    \begin{tabular}{|l|p{10cm}|} \hline 
     {\bf Stage} & {\bf Description} \\ \hline
...
\section{Mapping of Data from Spreadsheets to RDF}
\label{sec:csvtordf}

The preferred format of the data published on the semantic web is an \ac{rdf} format. 
Many scientists store and publish data using CSV, rather than \ac{rdf},
format because conversion of data to \ac{rdf} 
is comparatively difficult. 
A number of open source tools~\cite{ermilov2013csv},
listed on the \ac{w3c} 
wiki\footnote{\tt https://www.w3.org/wiki/ConverterToRdf},
can be used to do the conversion. Two worth highlighting are OpenRefine and RML Mapper.

OpenRefine\footnote{\tt https://openrefine.org},  
    is a Java-based set of tools that work with messy data using \ac{grel}
    It allows users to extract, transform, export, and convert 
    ...
    The RDF mapping is accomplished with an extension. 
   
``An \ac{rml} mapping defines a mapping from any logical source to \ac{rdf}. 
It is a structure that consists of one or more triples maps.''~\cite{dimou_ldow_2014}
%\footnote{\tt https://rml.io/specs/rml/\#example-XML} 

The \ac{rml} rules can be written and tested in YARRRML~\cite{yarrrml2018} 
(a more accessible way to use RML)
using tools like the Matey Web UI\footnote{\tt https://rml.io/yarrrml/matey/}.
...
The building blocks discussed here are used
to define a generic process for publishing \ac{lod} from spreadsheet data (Chapter~\ref{chap:methodology}),
which is then applied to a real example with Hepting's student feedback data
