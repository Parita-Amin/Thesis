...
A \ac{uri} comprises the five following components sequentially (see Fragment~\ref{generic-uri}. 
...
\noindent
Consider the \ac{uri} (in Sentence~\ref{example-uri}).
Figure~\ref{components-of-uri-example} 
shows its components.
??? Typing this \ac{uri} into a web browser (details of your browser) leads to ... ??? 
...
??? relate this to subject predicate object... ???
Each data object in a dataset on the web is considered a resource and is named 
using a \ac{uri}. 
To search for the resources on the web, 
the resources must be named using \ac{https} \acp{uri}. 
??? OR resources available on the web must be named using \ac{https} \acp{uri} ???
The \ac{w3c} has defined the 
??? a table/figure/something is needed to list the prefixes rdf:, etc. ???
{\tt rdf:type} 
...
called {\tt property} ??? {\em properties} REPETITION ???. 
...
as triples in subject-predicate-object form ??? use a real format for this ???,
or in graphical form.

??? Introduce the source of the pizza examples ???
...
\begin{figure}[h]
	\framebox{\vbox{\tt\small
        ...
        \caption{An illustration of Fragment~\ref{caloric-content} in a \ac{rdf}/\ac{xml} document.
        ??? Add prefixes to this code example ???}
        \label{rdf-xml-doc}
 \end{figure}
    ...
    The document consists of various \ac{xml} tags.
%    enclosed between
%    angle brackets (``<'' and ``>''). 
    An \ac{rdf}/\ac{xml} 
    ...
    and the object is {\tt xsd:integer} ??? use an accepted format for this ???. 
    ...
    \begin{figure}[htp]
        ...
        \caption{An illustration of Fragment~\ref{caloric-content} as an \ac{rdf} triple.
        ??? use a real example from the tutorial ???}
        \label{rdf-triple}
    \end{figure}
  
    Figure~\ref{rdf-graph} 
    shows Fragment~\ref{caloric-content} in graphical format. 
    ...
    to show that the property (predicate) connects the resource (subject) with the value (object).
    
    \begin{figure}[htp]
        \centering
        \includegraphics[width=\columnwidth]{images/ch2/rdf-graph.png}
        \caption{Graphical Representation of of Fragment~\ref{caloric-content} as an \ac{rdf} graph.}
        \label{rdf-graph}
    \end{figure}
...
%    the predicate being the third entity acts as a connector between them. 
    ...
    \ac{turtle}; \ac{n3}, 
    which is a subset of Turtle and N-Quads, ??? CHECK THESE ???
...
There can be more details added to the preceding \ac{rdf} examples 
(Figures~\ref{rdf-xml-doc},~\ref{rdf-triple}, and~\ref{rdf-graph}). 
That is, 
more statements can be added that are in relation to the original statement.  
...
\begin{figure}[htp]
    ...
    \caption{\ac{rdf} Notations for Figure~\ref{graph-for-relative-statements}, after \ac{rdf} Primer~\cite{manola2004rdf} ??? Would triples be this generic? Use real syntax (no line numbers) ???}
    \label{rdf-notations}
\end{figure}
...
is to be designed. --- TOO GENERIC What is this sentence meant to convey ???
...
\begin{flalign}  
    \text{\tt Pizza has PizzaBase} &&
    \text{\tt Romeo and Juliet is written by William Shakespeare} &&
\end{flalign}
