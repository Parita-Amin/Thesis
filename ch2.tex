...
\par Figure~\ref{rdf-triple} shows an example of an \ac{rdf} as a triple. An \ac{rdf} triple, generally, follows subject-predicate-object format. For the illustration in the figure, the subject is \say{https://www.semanticweb.org/pda324/ontologies/survey}, predicate is \say{includes} and the object is \say{Survey Details}.
\begin{figure}[htp]
    \centering
...
\par Figure~\ref{rdf-graph} show an \ac{rdf} in graphical format. Resource is indicated using an oval shape and a rectangle is used to represent a property value. A connector (an arrow) is used to represent a property to show that property (predicate) connects the resource (subject) with the value (object).
\begin{figure}[htp]
    \centering
...
\par Figure~\ref{rdfs-graph-for-complex} shows a graph for the representation of \ac{rdfs} for a comparatively complex and abstract example based on \ac{rdf} Primer~\cite{manola2004rdf}. The graph has been drawn for the following statement:
\emph{\begin{center}
...
\par In the form of \ac{rdf} triple, the statement in Figure~\ref{statement-for-rdf} can be represented as shown in Figure~\ref{triple-for-rdf-statement}. \ac{uri} is assigned to the subject survey.html and to the object which is the response with responseID 1234 which corresponds to the property value Robert James. A \ac{uri} is also assigned to the predicate (respondent).
\begin{figure}[htp]
    \centering
...
\par The new graph would be as shown in Figure~\ref{graph-for-relative-statements} where as the set of statements in Table~\ref{rdf-notations} shows the \ac{rdf} triple notation for Figure~\ref{graph-for-relative-statements}. The first notation represents the entities, survey.html (subject) and Robert James (object), along with respondent (predicate) as the relation between them, using the \ac{uri}s. The second notation is for the date, where instead of the \ac{uri}, the value of the property, Sept. 08, 2020, is used. Here, survey.html is the subject and Sept. 08, 2020 is the object with date acting as the predicate.
\begin{figure}[htp]
    \centering
...
\par Consider a more complex system, where the details of respondent are collected. Suppose, one of the details is name and the other is address of the respondent.  Figure~\ref{graph-for-respondent-details} shows a graphical representation of this \ac{rdf} system.
\begin{figure}[htp]
    \centering
...
\par A syntax diagram can be modelled to represent the components of Generic Syntax\footnote{https://www.w3.org/Addressing/URL/uri-spec.html} of \ac{uri} along with their hierarchy as shown in Figure~\ref{syntax-diagram}. Consider the following \ac{url}. Figure~\ref{components-of-url-example} shows its components.
\begin{center}
...
\par Figure~\ref{owl-properties} shows representation of properties between the individuals \say{Max} and \say{India}, and \say{Max} and \say{hasSibling}. The relation between \say{Max} and \say{India} is \say{livesIn}. The arrow between two individuals in the figure denotes the property (relationship) between them.
\begin{figure}[htp]
    \centering
...
\par Figure~\ref{owl-classes-with-individuals-and-properties} shows the representation of the classes for the individuals and properties that are shown in Figure~\ref{owl-properties}.
\begin{figure}[htp]
    \centering
...
\par For instance, the University of Regina, a university in the city of Regina (SK, Canada), has an item page allotted for it. Figure~\ref{wikidata-uofr} shows the Wikidata item page of the University of Regina (https://www.wikidata.org/wiki/Q3104287).
 \begin{figure}[htp]
    \centering
...
\par The \ac{uri} for an entity on Wikidata is \say{https://www.wikidata.org/wiki/<id>}, where <id> stands for the unique identifier. For the University of Regina, the <id> is the label identifier—Q3104287. Figure~\ref{rdf-xml-uofr} shows the \ac{rdf}/\ac{xml} description of the University of Regina on Wikidata.
\begin{figure}[htp]
    \centering
