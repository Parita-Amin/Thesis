...
\section{Modeling Ontology for Survey Using Protégé}
...
The ontology vocabularies can be queried using query languages like DL query\footnote{https://protegewiki.stanford.edu/wiki/DL\_Query} and \ac{sparql}. Protégé offers a plugin for DL as well as SPARQL queries which are discussed in this section. 

\subsection{DL Query}

Vasilevsky\footnote{https://ontology101tutorial.readthedocs.io/en/latest/EXERCISE\_BasicDL\_Queries.html} defines DL query as a class expression that is constructed using constructs such as `and’ (corresponding to set intersection) and `some’. Users can rapidly check that class definitions encompass the right subclasses by running them via the DL Query tab. Alternately, one can examine arbitrary descriptions for class membership without having to build placeholders for specified classes. The DL Query tab can be opened using Tabs option under Windows menu. It is important to configure a reasoner to successfully express a DL query. Figure~\ref{survey-dl-query} shows expression of a sample DL query on the survey ontology in protégé. 
...
According to \ac{w3c}~\cite{garlik2013sparql}, \ac{sparql} queries use conjunctions, disjunctions, aggregation, subqueries, negation, and other operations to define queries and optional graph patterns. Protégé offers plugins for \say{\ac{sparql} Query} and \say{Snap \ac{sparql} Query}.

\subsubsection{Using \ac{sparql} Query Plugin}
    
\ac{sparql} plugin in protégé is used to express simple queries on ontologies like displaying classes and subclasses. For instance, consider the following query:
...
\end{lstlisting}
 ...
 Figure~\ref{survey-sparql-query} shows the result of expression of \ac{sparql} query on survey ontology. Here, the query is used to display \say{Class} and \say{subClassOf} with column titles subject and object respectively.
...
The Snap \ac{sparql} plugin in protégé is generally used to express comparatively complex queries on ontologies like displaying description of individuals. For instance, consider the following query:    
...
Figure~\ref{survey-snap-sparql-query} shows the result of expression of Snap \ac{sparql} query on survey ontology. It is necessary to enable a reasoner for successful working of Snap \ac{sparql} query. Here, the query is used to display \say{Class} and \say{Individual} with column titles ?Class and ?Individuals respectively.
...
Primarily, OpenRefine\footnote{https://openrefine.org/} has been used to convert the data stored in spreadsheets to \ac{turtle} and \ac{rdf} files using the methodology described in section~\ref{sec:csvtolod}. Using survey ontology in OpenRefine, the \ac{csv} file consisting of survey responses is converted into \ac{turtle} and \ac{rdf}/\ac{xml} files.

After importing the \ac{csv} and creating a new project, the next step is to manage \ac{rdf} Schema alignment to describe the data stored in \ac{csv} in terms of linked data vocabulary. On selecting \say{Edit \ac{rdf} Skeleton} from the menu of \ac{rdf} Extension, an \say{\ac{rdf} Schema alignment} dialog box appears which offers two options: \ac{rdf} Skeleton and \ac{rdf} Preview. The base \ac{uri} for the \ac{rdf} schema alignment has been set to:
...
Figure~\ref{openrefine-rdf-skeleton} shows \ac{rdf} skeleton for \ac{csv} using survey ontology. It consists of a list of prefixes along with options for adding and managing prefixes; \ac{uri} for index count; and the fields in the project with an option of using linked data vocabularies to describe them
...
\subsubsection{Prefixes}
    ...
    \begin{itemize}
        ...
        \item It also allows adding other prefixes which can be done by clicking on \say{+Add} option as shown in Figure~\ref{openrefine-add-prefix}. The prefix \say{survey} has been added to the list with \ac{uri} https://parita-amin.github.io/survey.owl\#.
...
\subsubsection{RDF Node for Index}
    ...
    Index is a unique identifier for the number of records in the table. Figure~\ref{openrefine-index} shows options available for the settings of the \ac{rdf} Node for index. For survey data, \ac{uri} for \say{ID} has been set as unique index.
...
As mentioned in section~\ref{sec:announcinglod}, it is necessary to specify an appropriate license for linked open data. When there is a clear declaration regarding the source, ownership, and conditions associated to the use of the public data, data reuse is more likely to happen.

\begin{itemize}
    
    \item Publishing Survey Vocabulary:
    
    The linked open vocabulary for feedback survey that Dr. Hepting solicits from his students in the classes he teaches. The dataset can be accessed as \ac{turtle} file from my website\footnote{https://parita-amin.github.io/survey.ttl} or can be viewed by visiting the link to the \ac{turtle} file\footnote{https://parita-amin.github.io/evaluationofteaching.html} on my website (see Figure~\ref{survey-license}).
